...
# =================================================================
# SAMBA 4 ACTIVE DIRECTORY TARTOMÁNYVEZÉRLŐ TELEPÍTŐ SCRIPT (1.0b)
# Támogatott rendszerek: Debian 13 (Trixie), Ubuntu 22.04 LTS, Ubuntu 25.04
# =================================================================
...
echo "Debian/Ubuntu Samba AD DC Installer - Robusztus Verzió"
echo "Támogatott: Debian 13, Ubuntu 22.04 LTS, Ubuntu 25.04"
echo "================================================================" 
...
# --- RENDSZER FELISMERÉS ÉS KONFIGURÁCIÓ ---

detect_system() {
    section_header "RENDSZER FELISMERÉS"
    ...
    if [ -f /etc/os-release ]; then
        ...
        info_msg "Rendszer: $NAME $VERSION ($VERSION_CODENAME)"
        
        # Támogatott rendszerek ellenőrzése
        case "$SYSTEM_ID" in
            "debian")
                if [ "$SYSTEM_VERSION" -lt 12 ]; then
                    warn_msg "Debian 12-nél régebbi verzió. Kompatibilitás nem garantált."
                fi
                ;;
            "ubuntu")
                case "$SYSTEM_VERSION" in
                    "22.04"|"25.04")
                        info_msg "Ubuntu $SYSTEM_VERSION LTS támogatott"
                        ;;
                    *)
                        warn_msg "Ubuntu $SYSTEM_VERSION - tesztelésre ajánlott"
                        ;;
                esac
                ;;
            *)
                warn_msg "Nem támogatott disztribúció: $SYSTEM_ID"
                read -p "$(echo -e "${CLR_DEBIAN_YELLOW}Folytatja? (i/n): ${CLR_RESET}")" -n 1 -r
                echo
                if [[ ! $REPLY =~ ^[Ii]$ ]]; then
                    exit 1
                fi
        ...
        error_msg "/etc/os-release nem található. A rendszer felismerése sikertelen."
        exit 1
    fi
}

# --- DISTRIBÚCIÓ SPECIFIKUS BEÁLLÍTÁSOK ---

configure_system_specific() {
    section_header "DISTRIBÚCIÓ SPECIFIKUS BEÁLLÍTÁSOK"
    
    case "$SYSTEM_ID" in
        "debian")
            info_msg "Debian specifikus beállítások..."
            # Debian alapértelmezett beállítások - nincs extra konfiguráció szükséges
            ;;
        "ubuntu")
            info_msg "Ubuntu specifikus beállítások..."
            
            # AppArmor profil ellenőrzése Samba-hoz
            if [ -f /etc/apparmor.d/usr.sbin.smbd ]; then
                info_msg "AppArmor profil ellenőrzése..."
                aa-status | grep -q samba && info_msg "AppArmor Samba profil aktív" || warn_msg "AppArmor Samba profil nem aktív"
            fi
            
            # NetworkManager konfiguráció ellenőrzése
            if systemctl is-active --quiet NetworkManager; then
                warn_msg "NetworkManager aktív. Ellenőrizze a DNS beállításokat."
            fi
            
            # Ubuntu 22.04 specifikus beállítások
            if [ "$SYSTEM_VERSION" = "22.04" ]; then
                info_msg "Ubuntu 22.04 LTS specifikus beállítások..."
                # Extra csomagok Ubuntu 22.04-hoz
                UBUNTU_EXTRA_PACKAGES="python3-markdown-it"
            fi
            
            # Ubuntu 25.04 specifikus beállítások  
            if [ "$SYSTEM_VERSION" = "25.04" ]; then
                info_msg "Ubuntu 25.04 specifikus beállítások..."
                # Jövőbeli Ubuntu verzió előkészítése
...
# --- CSOMAGKEZELŐ FUNKCIÓK ---

get_package_list() {
    # Alap Samba csomagok minden disztribúcióhoz
    local BASE_PACKAGES="samba-ad-dc krb5-user dnsutils python3-setproctitle net-tools"
    ...
    case "$SYSTEM_ID" in
        "debian")
            ...
            ;;
        "ubuntu")
            local UBUNTU_PACKAGES="$BASE_PACKAGES"
            if [ -n "$UBUNTU_EXTRA_PACKAGES" ]; then
                UBUNTU_PACKAGES="$UBUNTU_PACKAGES $UBUNTU_EXTRA_PACKAGES"
            fi
...
configure_package_manager() {
    section_header "CSOMAGKEZELŐ BEÁLLÍTÁSA"
    
    case "$SYSTEM_ID" in
        "ubuntu")
            # Ubuntu esetén biztosítjuk a universe repository-t
            if ! grep -q "^deb.*universe" /etc/apt/sources.list; then
                info_msg "Universe repository hozzáadása..."
...
# --- JAVÍTOTT FUNKCIÓK (Multi-Distro Kompatibilitás) ---
...
# --- ÚJ FUNKCIÓ: RENDSZER SPECIFIKUS DNS BEÁLLÍTÁS ---

configure_system_dns() {
    section_header "DNS KONFIGURÁCIÓ"
    ...
    # Meglévő symlink eltávolítása
    [ -L /etc/resolv.conf ] && unlink /etc/resolv.conf
    
    # Immutable flag eltávolítása (ha létezik)
    chattr -i /etc/resolv.conf 2>/dev/null || true
    
    case "$SYSTEM_ID" in
        "ubuntu")
            # Ubuntu esetén különösen óvatosan kell eljárni
            warn_msg "Ubuntu rendszer - DNS konfiguráció érzékeny"
            
            # Systemd-resolved ellenőrzése és kezelése
            if systemctl is-active --quiet systemd-resolved; then
                warn_msg "Systemd-resolved aktív. Letiltás..."
                systemctl stop systemd-resolved
                systemctl disable systemd-resolved
                systemctl mask systemd-resolved
            fi
            
            # NetworkManager ellenőrzése
            if systemctl is-active --quiet NetworkManager; then
                warn_msg "NetworkManager aktív. Manuális DNS beállítás szükséges."
                info_msg "Ha probléma adódik, állítsa be manuálisan a DNS-t NetworkManager-en keresztül"
            fi
            ;;
        "debian")
            # Debian - systemd-resolved ellenőrzése
            if systemctl is-active --quiet systemd-resolved; then
                warn_msg "Systemd-resolved aktív. Letiltás..."
                systemctl stop systemd-resolved
                systemctl disable systemd-resolved
...
# --- KERBEROS KONFIGURÁCIÓ JAVÍTÁSA ---
...
# --- TŰZFAL KONFIGURÁCIÓ (Multi-Distro) ---

configure_firewall() {
    section_header "TŰZFAL KONFIGURÁCIÓ"
    
    # UFW (Ubuntu és néhány Debian)
    ...
    # iptables/firewalld ellenőrzése
    if command -v iptables >/dev/null 2>&1; then
        warn_msg "iptables aktív lehet. Ellenőrizze a szükséges portokat manuálisan."
    fi
    
    if command -v firewalld >/dev/null 2>&1; then
        warn_msg "firewalld aktív lehet. Ellenőrizze a szükséges portokat manuálisan."
    fi
}
...
echo "Rendszer: $SYSTEM_ID $SYSTEM_VERSION"
...
# --- 7. DISTRIBÚCIÓ SPECIFIKUS BEÁLLÍTÁSOK ---
configure_system_specific
...
SAMBA_PACKAGES=$(get_package_list)
...
# --- 9. TARTOMÁNY LÉTREHOZÁSA (PROVISIONING) ---
...
# --- 12. VÉGLEGES TESZTELÉS ÉS BEFEJEZÉS ---
...
echo -e "${CLR_DEBIAN_GREEN}🎉 SAMBA AD DC TARTOMÁNYVEZÉRLŐ SIKERESEN TELEPÍTVE!${CLR_RESET}"
echo -e "${CLR_DEBIAN_GREEN}Rendszer: $SYSTEM_ID $SYSTEM_VERSION${CLR_RESET}"
echo -e "${CLR_DEBIAN_GREEN}=================================================================${CLR_RESET}"
echo ""
echo "📋 TELEPÍTÉSI ÖSSZEFOGLALÓ:"
echo -e "    • Rendszer: ${CLR_DEBIAN_BLUE}$SYSTEM_ID $SYSTEM_VERSION${CLR_RESET}"
...
if command -v reboot >/dev/null 2>&1; then
    ...
    warn_msg "A reboot parancs nem elérhető. Kézzel indítsa újra a rendszert!"
    echo "Használja: systemctl reboot vagy shutdown -r now"
fi
